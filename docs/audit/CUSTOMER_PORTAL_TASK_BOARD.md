...
|---|---|---|---|---|---|---|---|---|
| CP-001 | Navigation (Header + MobileNav) | UX Bug | Add discoverable access for `/schedule` and `/statement` (desktop + mobile). | High | Agent C | READY | - | User can reach both pages within max 2 taps from any portal page. |
...
| CP-006 | Deliveries + Schedule date handling | Bug Fix | Replace timezone-sensitive date string generation with local-safe formatter (`YYYY-MM-DD` without UTC drift). | High | Agent C | READY | - | Month filter and schedule range return expected boundary dates in PK timezone. |
| CP-007 | Auth (`use-auth.ts`, `middleware.ts`) | Security UX | Enforce CUSTOMER role at login/middleware level; handle wrong-role tokens with clean redirect/logout. | High | Agent C | READY | - | Non-customer token cannot remain inside customer portal shell. |
| CP-008 | Transactions page UX | Responsive UX | Add mobile card/list mode for transactions; keep table for desktop only. | Medium | Agent C | READY | - | Mobile (<640px) has readable transaction cards without horizontal table overflow. |
| CP-009 | Orders page UX | Feature Gap | Add status filter tabs/chips (All/Pending/Approved/Rejected/Cancelled) with page reset. | Medium | Agent C | READY | - | Status filter updates query and resets page to 1. |
| CP-010 | Support page UX | Feature Gap | Add ticket status filter (All/Open/In Progress/Resolved/Closed) + clearer reply badge/preview. | Medium | Agent C | READY | - | User can quickly isolate unresolved tickets and identify replied tickets. |
| CP-011 | Profile (`profile-card.tsx`) | Bug Fix | Add Sunday (`7`) day label mapping and ensure schedule day labels consistent across app. | Low | Agent C | READY | - | Day labels display correctly for all 1..7 days. |
