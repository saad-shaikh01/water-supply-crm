...
| CP-005 | Transactions (`transaction-list.tsx`) | Data Correctness | Remove misleading client-side type filtering over paginated data; either use server-side filter or relabel fallback clearly. | High | Agent C | READY | API-014 | Dependency cleared: API-014 implemented. Proceed with FE wiring + UX validation. |
...
| ID | Route / Module | Type | Task | Priority | Owner | Status | Depends On | Acceptance |
|---|---|---|---|---|---|---|---|---|
| CP-015 | Transactions API | API + FE | Add server-side filters (`type`, `dateFrom`, `dateTo`, `search`) and wire compact filter bar. | Medium | Agent C | READY | API-014 | Dependency cleared: API-014 implemented. Proceed with compact filter bar wiring. |
| CP-016 | Orders lifecycle | API + FE | Expose richer order fulfillment states (`PLANNED`, `OUT_FOR_DELIVERY`, `DELIVERED`) and show timeline in portal. | Medium | Agent C | READY | API-015 | Dependency cleared: API-015 implemented. Proceed with lifecycle timeline UI. |
| CP-017 | Tickets enhancements | API + FE | Add ticket conversation timeline + optional attachment support for customer/vendor replies. | Medium | Agent C | READY | API-016 | Dependency cleared: API-016 implemented. Proceed with timeline + attachment UX. |
| CP-018 | Product pricing for portal orders | API + FE | Return customer effective price in portal product payload and show in Place Order dialog. | Medium | Agent C | READY | API-017 | Dependency cleared: API-017 implemented. Proceed with effective price rendering. |
