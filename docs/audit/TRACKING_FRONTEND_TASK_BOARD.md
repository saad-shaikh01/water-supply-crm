...
| TRK-FE-005 | Tracking UX | High | READY | Show freshness semantics on markers/popups (`LIVE`, `STALE`, `OFFLINE`) using backend freshness metadata; display `last seen` explicitly. | TRK-BE-003 |
| TRK-FE-006 | Tracking UX | High | READY | Add selected-driver side drawer with: driver, van, sheet, speed, bearing, last update, and quick links to sheet/history screens. | TRK-BE-007 |
