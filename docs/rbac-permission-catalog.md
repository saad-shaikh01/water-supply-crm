...
> **Amendment R8 (Sheet Discrepancy Case, owner-approved 2026-08-18):** added the new **`sheet_discrepancies`** resource (navigable — `/dashboard/discrepancy-cases` is a dedicated review queue), 3 actions covering the resolution flow for bottle/empty/cash reconciliation gaps auto-detected by `DailySheetService.closeSheet()`: `page`, `view`, `resolve`. No `create` action — cases are exclusively system-generated at sheet-close time, never user-initiated (unlike `damage_cases:create`). `resolve` covers all three resolution outcomes (charge to driver, company loss write-off, waived) as one action rather than split per-outcome the way `damage_cases` splits `charge`/`waive`/`reverse` — the feature's requirement is a single resolution authority for all three. Default holders: `VENDOR_ADMIN` (wildcard) and `STAFF`/Manager (added to `MANAGER_PERMISSIONS` in `presets.ts`, flat default, no override-only tier the way `payroll:view_all` has). This amendment also corrects `permissions.spec.ts`'s frozen-contract constants, which had drifted out of sync since `fleet` (Amendment R7, 2026-08-10) was added without updating them — the true pre-R8 baseline was 157 permissions/28 pages/29 resources, not the 150/26/28 the file still claimed. **Totals updated: 160 permissions, 30 resources.**

> **Amendment R9 (Daily Sheet Soft Close, owner-approved 2026-08-18):** added 3 actions to the existing **`daily_sheets`** resource (no new resource, no new `:page` — already navigable): `request_close` (Driver/Salesman self-closes their own sheet — `isClosed` flips true immediately, same edit-lock as today's direct close, but the Crew Cash → Payroll Ledger sync and Sheet Discrepancy Case creation are deferred), `approve_close` (Staff/Admin approves the request — runs that deferred sync/case-creation), `reject_close` (Staff/Admin sends it back — reopens the sheet, `isClosed=false`, for the requester to fix and resubmit). The existing `close` action is unchanged — a direct Staff/Admin close still skips the review cycle entirely. Default holders: `request_close` → `DRIVER`, `SALESMAN` (added to their presets in `presets.ts`); `approve_close`/`reject_close` → `VENDOR_ADMIN` (wildcard) and `STAFF`/Manager (added to `MANAGER_PERMISSIONS` alongside the existing `close`), same tier as `close` itself. This feature also makes the end-of-day `VehicleDailyCheck` mandatory before *any* close path (direct, request, or approve) — mirrors the Fleet Phase 1 start-of-day gate on trip start (`vehicle-check.service.ts`'s `assertTripEndClear`), not a new permission (folds into the existing `daily_sheets:close`/`request_close`/`approve_close` gates). **Totals updated: 163 permissions, 30 resources.**

> **Amendment R10 (owner-requested 2026-08-20):** added **`daily_sheets:move_customer`** to the existing `daily_sheets` resource (no new resource, no new `:page`), gating `PATCH /daily-sheets/items/move` — moving one or more customers' pending/failed deliveries to a different van/sheet. Previously this rode along with the broad `daily_sheets:update` grant, which meant every role holding `:update` (including Driver and Salesman) could also move customers with no way to restrict it independently. Split out so it's its own toggle in the permission-management UI. Default holders: `VENDOR_ADMIN` (wildcard) and `STAFF`/Manager (added to `MANAGER_PERMISSIONS` in `presets.ts`, preserving pre-R10 behavior for that tier). **Not** granted by default to `DRIVER` or `SALESMAN` anymore — a vendor must explicitly tick it on that role in Roles & Access if field roles should be able to move customers themselves. **Note:** because existing (already-seeded) per-vendor system roles are never silently re-synced to preset changes (`rbac-seed.ts`: "preset permissions are written ONLY when the role is newly created"), vendors upgrading past this amendment will see Move disappear for whichever roles used to reach it only via `:update` — including Manager/Staff — until an admin re-ticks `daily_sheets:move_customer` on those roles (or uses "Reset to preset" on system roles that haven't been customized). **Totals updated: 164 permissions, 30 resources.**
...
| `daily_sheets:close` | `POST /daily-sheets/:id/close` (direct Staff/Admin close — skips the request/approve review cycle) |
| `daily_sheets:request_close` | `POST /daily-sheets/:id/request-close` (Driver/Salesman self-close — Amendment R9) |
| `daily_sheets:approve_close` | `POST /daily-sheets/:id/approve-close` (Staff/Admin approves a self-close request — Amendment R9) |
| `daily_sheets:reject_close` | `POST /daily-sheets/:id/reject-close` (Staff/Admin rejects, reopening the sheet — Amendment R9) |
...
| `daily_sheets:edit_closed_trip` | `PATCH /daily-sheets/:id/loads/:loadId/correct-checkin` (amend a checked-in load trip's physical counts on an already-closed sheet — Amendment R12, owner-requested 2026-09-02) |
| `daily_sheets:record_walk_in` | `POST /daily-sheets/walk-in` (record a delivery made off the route pipeline — owner-requested 2026-09-04) |
| `daily_sheets:edit_closed_expense` | `PATCH /expenses/:id/correct`, `POST /expenses/:id/void`, `POST /expenses/closed` (edit / hard-delete / add an Expense row on an already-closed sheet — Amendment R13, owner-requested 2026-09-07) |
...
> **Amendment R12 (Post-Close Trip Correction, owner-requested 2026-09-02).** New action `daily_sheets:edit_closed_trip`, default holders **Admin + Manager** (analogous to `void_delivery`; added to `MANAGER_PERMISSIONS` + a `PRESET_DRIFT_BACKFILLS.manager` catch-up for existing vendors, `manager.allow` / `salesman.deny` / `driver.deny` in the enforcement matrix mirroring `void_delivery` exactly). Gates a dedicated endpoint `PATCH /daily-sheets/:id/loads/:loadId/correct-checkin` that amends an already-checked-in load trip's physical counts (returned filled / collected empty / damaged / leaked on van) on a **closed** sheet — it does NOT relax `checkinLoad`'s own `isClosed` guard (that path stays closed-sheet-blocked). Applies the signed deltas through the same warehouse ledger (`recordCheckinCorrection`) and sheet-aggregate adjustments as a normal trip-edit; a mandatory free-text `correctionNote` is stored in the audit `after` block. Like Void Delivery, it accepts the same PDF/reconciliation divergence — the live bottle/empty recompute moves but the frozen close-time `cashExpected` and any `SheetDiscrepancyCase` rows are left untouched. No new `:page` — `daily_sheets` was already navigable. Frozen total 167 → 168; non-page total 98 → 99.

> **Amendment R13 (Post-Close Expense Correction, owner-requested 2026-09-07).** New action `daily_sheets:edit_closed_expense`, default holders **Admin + Manager** (analogous to `edit_closed_trip`; added to `MANAGER_PERMISSIONS` + a `PRESET_DRIFT_BACKFILLS.manager` catch-up for existing vendors, `manager.allow` / `salesman.deny` / `driver.deny` in the enforcement matrix mirroring `void_delivery` / `edit_closed_trip` exactly). Gates three dedicated endpoints hosted on `ExpenseController` — `PATCH /expenses/:id/correct` (amend a closed-sheet Expense's mutable fields), `POST /expenses/:id/void` (**hard-delete** it — `tx.expense.delete`, the audit `before` block is the only surviving record), `POST /expenses/closed` (add a missed Expense onto a closed sheet). Each `SELECT … FOR UPDATE`-locks the row, re-asserts the sheet is still closed in-txn, increments a new marker column `DailySheet.postCloseExpenseCorrectionCount`, and writes a `CLOSED_EXPENSE_CORRECTED` / `CLOSED_EXPENSE_VOIDED` / `CLOSED_EXPENSE_ADDED` audit row carrying the mandatory free-text `correctionNote`. An Expense linked to a `FuelLog` / `VehicleServiceRecord` / `SheetDiscrepancyCase` is rejected (`ConflictException`). Like the two siblings it accepts the frozen-snapshot divergence — the close-time `cashExpected` / `cashCollected` and any `SheetDiscrepancyCase` rows are left untouched; the post-close divergence banner + hybrid cash rollups surface it. The ordinary `PATCH /expenses/:id` and `DELETE /expenses/:id` now also refuse a closed-sheet expense (previously a latent hole) and point the caller at these endpoints. No new `:page` — `daily_sheets` was already navigable. Frozen total 169 → 170; non-page total 100 → 101.
...
### 29. Sheet Discrepancy Cases — `sheet_discrepancies` *(navigable — `/dashboard/discrepancy-cases`)*
> Cases are exclusively system-generated inside `DailySheetService.closeSheet()` / `approveClose()` (Amendment R9: the latter is the Soft Close equivalent, run when Staff/Admin approves a Driver/Salesman's self-close request rather than at direct close) — one row per non-zero bottle/empty/cash reconciliation gap, no auto-waive threshold — there is no `create`/report route, unlike `damage_cases`. `resolve` covers all three resolution outcomes (`CHARGED_TO_DRIVER` → `StaffLedgerEntry` PENALTY debit against the driver, `COMPANY_LOSS` → an `Expense` write-off row, `WAIVED` → no money moves) as one flat action, not split per-outcome.

| Permission | Gates |
|---|---|
| `sheet_discrepancies:page` | `/dashboard/discrepancy-cases` route access |
...
> **Amendment R16 (Staff Attendance & Wage Types Phase 1, owner-approved 2026-09-11).** Two new actions on the existing `payroll` resource (no new resource, no new `:page` — an attendance screen lives under `/dashboard/payroll` and inherits `payroll:page`): `attendance_view` (the vendor-wide attendance grid; self-view of one's own attendance needs no permission, code-level scope check like `payroll:view_all`) and `attendance_mark` (manual per-employee-per-day marking — an `ABSENT` / `HALF_DAY` marking also creates one `LEAVE_UNPAID` `StaffLedgerEntry` in the same transaction, via `StaffLedgerService.createTx`; the amount is an explicit caller input, no per-day divisor logic). Default holders **Admin + Manager** (added to `MANAGER_PERMISSIONS` + a `PRESET_DRIFT_BACKFILLS.manager` catch-up for existing vendors; `manager.allow` / `salesman.deny` / `driver.deny` / `viewer.deny` in the enforcement matrix). Attendance rows are auto-captured (status `PRESENT`, source `CREW_CONFIRM`) inside `DailySheetService.confirmCrew`'s transaction; attendance itself is operational data with no audit-log table of its own — the only financial consequence is the linked ledger entry, which carries its own `StaffLedgerAuditLog`. Frozen total 178 → 180; non-page total +2.
>
