...
> Cases are exclusively system-generated inside `DailySheetService.closeSheet()` (one row per non-zero bottle/empty/cash reconciliation gap, no auto-waive threshold) — there is no `create`/report route, unlike `damage_cases`. `resolve` covers all three resolution outcomes (`CHARGED_TO_DRIVER` → `StaffLedgerEntry` PENALTY debit against the driver, `COMPANY_LOSS` → an `Expense` write-off row, `WAIVED` → no money moves) as one flat action, not split per-outcome.
