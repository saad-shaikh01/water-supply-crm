...
6a. (Amendment) The START check itself is mandatory, not just its critical items — matching §7.2's original "Mandatory" intent. `createLoad`/`loadOut` hard-block (409) with no `VehicleDailyCheck(checkType=START)` recorded for the sheet at all, exactly like the `crewConfirmed` gate. Only *rule 6* above (critical checklist items) was ever meant to be the exception; a missing check entirely was never meant to be optional.
