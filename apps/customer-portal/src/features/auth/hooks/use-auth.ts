import { useMutation, useQueryClient } from '@tanstack/react-query';
import { setCookie, deleteCookie, getCookie } from 'cookies-next';
import { useRouter } from 'next/navigation';
import { toast } from 'sonner';
import { authApi, type LoginResponse } from '../api/auth.api';
import { useAuthStore } from '../../../store/auth.store';
import type {
  LoginInput,
  ActivationEligibilityInput,
  ActivationPasswordInput,
} from '../schemas';
import { unregisterSessionFcmToken } from '../../notifications/lib/fcm-session';

/** Shared by useLogin and useActivateAccount — both land the browser in an authenticated session. */
const useApplySession = () => {
  const router = useRouter();
  const queryClient = useQueryClient();
  const setUser = useAuthStore((s) => s.setUser);
  const clearUser = useAuthStore((s) => s.clearUser);

  return (data: LoginResponse) => {
    const customerId = data.user.customerId;
    if (data.user.role !== 'CUSTOMER' || !customerId) {
      unregisterSessionFcmToken().catch(() => null);
      authApi.logout(data.refresh_token).catch(() => null);
      deleteCookie('auth_token');
      deleteCookie('refresh_token');
      clearUser();
      queryClient.clear();
      toast.error('This account cannot access the customer portal');
      router.replace('/auth/login');
      return;
    }

    setCookie('auth_token', data.access_token, { maxAge: 60 * 60 * 24 });       // 1 day
    setCookie('refresh_token', data.refresh_token, { maxAge: 60 * 60 * 24 * 7 }); // 7 days
    setUser({
      id: data.user.id,
      name: data.user.name,
      email: data.user.email,
      customerId,
    });
    queryClient.setQueryData(['auth', 'me'], data.user);
    router.push('/home');
  };
};

export const useLogin = () => {
  const applySession = useApplySession();

  return useMutation({
    mutationFn: (data: LoginInput) => authApi.login(data),
    onSuccess: ({ data }) => applySession(data),
    onError: () => toast.error('Invalid email or password'),
  });
};

export const useCheckEligibility = () => {
  return useMutation({
    mutationFn: (data: ActivationEligibilityInput) => authApi.checkEligibility(data),
  });
};

type ActivateInput = ActivationEligibilityInput & Pick<ActivationPasswordInput, 'password'>;

export const useActivateAccount = () => {
  const applySession = useApplySession();

  return useMutation({
    mutationFn: (data: ActivateInput) => authApi.activate(data),
    onSuccess: ({ data }) => applySession(data),
  });
};

export const useResetPasswordWithCode = () => {
  const applySession = useApplySession();

  return useMutation({
    mutationFn: (data: ActivateInput) => authApi.resetPasswordWithCode(data),
    onSuccess: ({ data }) => applySession(data),
  });
};

export const useLogout = () => {
  const router = useRouter();
  const queryClient = useQueryClient();
  const clearUser = useAuthStore((s) => s.clearUser);

  return () => {
    const refreshToken = getCookie('refresh_token') as string | undefined;
    unregisterSessionFcmToken().catch(() => null);
    if (refreshToken) {
      authApi.logout(refreshToken).catch(() => null);
    }
    deleteCookie('auth_token');
    deleteCookie('refresh_token');
    clearUser();
    queryClient.clear();
    router.push('/auth/login');
  };
};
