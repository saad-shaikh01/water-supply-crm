...
import { CreditCard, QrCode, Upload, CheckCircle2, Loader2, Info, Banknote } from 'lucide-react';
import { usePaymentInfo, useInitiateRaastQr, useSubmitManualPayment } from '../hooks/use-payments';
...
export function PaymentDialog({ open, onOpenChange, suggestedAmount = 0 }: PaymentDialogProps) {
  ...
  const handleInitiateRaast = () => {
    ...
    initiateRaast({ amount: Number(amount) }, {
      onSuccess: (data) => {
        setQrResponse(data);
        if (data.checkoutUrl) {
        ...
        <div className="p-8">
          {!qrResponse ? (
            ...
            <div className="py-10 text-center space-y-6 animate-in fade-in zoom-in-95">
              <div className="h-20 w-20 rounded-full bg-emerald-500/10 border-4 border-emerald-500 flex items-center justify-center text-emerald-500 mx-auto">
                <CheckCircle2 className="h-10 w-10" />
              </div>
              <div className="space-y-2">
                <h3 className="text-2xl font-black">QR Generated</h3>
                <p className="text-sm text-muted-foreground max-w-[280px] mx-auto leading-relaxed">
                  We've opened the secure checkout page. Scan the QR code there to pay <span className="font-bold text-foreground">₨ {amount}</span>.
                </p>
              </div>
