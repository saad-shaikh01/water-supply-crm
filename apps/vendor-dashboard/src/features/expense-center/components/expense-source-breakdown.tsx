...
import { sourceBucketMeta } from '../constants';

export function ExpenseSourceBreakdown() {
  const { data: summary, isLoading } = useExpenseCenterSummary();

  if (isLoading) return <Skeleton className="h-24 rounded-2xl" />;
  if (!summary) return null;

  // A zero-amount source contributes nothing but noise — it would render a
  // 0px-wide bar segment and a "0%" legend row nobody can act on.
  const slices = (summary.bySource ?? []).filter((s) => Number(s.amount) > 0);

  if (slices.length === 0) {
    return (
      <div className="rounded-2xl bg-card/30 border border-border/40 p-4">
        <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Spend by Source</p>
  ...
  return (
    <div className="rounded-2xl bg-card/30 border border-border/40 p-4 space-y-3">
      ...
      <div className="flex h-3 w-full overflow-hidden rounded-full bg-muted">
        {slices.map((slice) => {
          const meta = sourceBucketMeta(slice.source);
          return (
            <div
              key={slice.source}
      ...
      <div className="flex flex-wrap gap-x-5 gap-y-2">
        {slices.map((slice) => {
          const meta = sourceBucketMeta(slice.source);
          return (
            <div key={slice.source} className="flex items-center gap-2 min-w-0">
