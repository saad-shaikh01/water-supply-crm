...
/**
 * `getCookie` can't see cookies during SSR/the very first client render (no `window` yet),
 * so gating `enabled` on it directly makes the query look "disabled" rather than "about to
 * run" on that first pass — even when the user is authenticated. Gating on a mount-only flag
 * first ensures cookies are only read once they're reliably readable.
 */
...
export function usePermissionsQuery() {
  ...
  return useQuery({
    ...
    enabled: hasMounted && !!getCookie('auth_token'),
    staleTime: 60_000,
  });
}
...
export function usePermissions(): UsePermissionsResult {
  ...
  return {
    ...
    // Before mount (SSR/first paint) or before the first `/auth/me` fetch resolves, there's
    // no way to tell "denied" from "not checked yet" — report loading so RouteGuard/Sidebar
    // show a spinner/skeleton instead of a false Access Denied on every hard refresh.
    isLoading: !hasMounted || isPending,
    isSuperAdmin,
