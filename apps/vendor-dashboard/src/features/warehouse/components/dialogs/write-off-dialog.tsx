...
import { useWriteOff } from '../../hooks/use-warehouse';
...
const defaultForm = { productId: '', quantity: 1, fromStock: 'damaged' as 'damaged' | 'leaked', notes: '' };

export function WriteOffDialog({ open, onOpenChange, products }: WriteOffDialogProps) {
  const { mutate, isPending } = useWriteOff();
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="rounded-3xl max-w-sm">
        <DialogHeader>
          <DialogTitle className="text-xl font-black">Write Off</DialogTitle>
        </DialogHeader>
        <div className="space-y-4 py-4">
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              {(['damaged', 'leaked'] as const).map((opt) => (
                <button
                  key={opt}
                  type="button"
