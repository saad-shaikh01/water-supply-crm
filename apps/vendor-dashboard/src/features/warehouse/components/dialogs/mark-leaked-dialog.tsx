...
import { useMarkLeaked } from '../../hooks/use-warehouse';
...
interface MarkLeakedDialogProps {
  open: boolean;
...
export function MarkLeakedDialog({ open, onOpenChange, products }: MarkLeakedDialogProps) {
  const { mutate, isPending } = useMarkLeaked();
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="rounded-3xl max-w-sm">
        <DialogHeader>
          <DialogTitle className="text-xl font-black text-red-500">Mark Leaked</DialogTitle>
        </DialogHeader>
