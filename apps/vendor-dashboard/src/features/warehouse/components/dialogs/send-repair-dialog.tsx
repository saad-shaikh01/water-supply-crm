...
import { useSendRepair } from '../../hooks/use-warehouse';
...
const defaultForm = {
  ...
  quantity: 1,
  shopName: '',
  fromStock: 'damaged' as 'damaged' | 'empty',
  notes: '',
};

export function SendRepairDialog({ open, onOpenChange, products }: SendRepairDialogProps) {
  const { mutate, isPending } = useSendRepair();
  ...
  const handleSubmit = () => {
    if (!form.productId || form.quantity < 1 || !form.shopName.trim()) return;
    mutate(
      {
        productId: form.productId,
        quantity: form.quantity,
        shopName: form.shopName.trim(),
        fromStock: form.fromStock,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="rounded-3xl max-w-sm">
        <DialogHeader>
          <DialogTitle className="text-xl font-black text-purple-500">Send for Repair</DialogTitle>
        </DialogHeader>
        <div className="space-y-4 py-4">
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              {(['damaged', 'empty'] as const).map((opt) => (
                <button
                  key={opt}
                  type="button"
        ...
        <DialogFooter>
          ...
          <Button
            ...
            disabled={isPending || !form.productId || form.quantity < 1 || !form.shopName.trim()}
