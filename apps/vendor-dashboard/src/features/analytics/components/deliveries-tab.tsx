...
import { Package, CheckCircle, XCircle, Activity, AlertTriangle, Repeat, Clock3, Droplets, RotateCcw, Archive, PackageCheck, PackageX } from 'lucide-react';
...
export function DeliveriesTab({ from, to, vanId }: { from: string; to: string; vanId?: string }) {
  ...
  const {
    delivered: bottlesDelivered = 0, returned: bottlesReturned = 0, filledReturned: filledBottlesReturned = 0,
    net: bottlesNet = 0, outstandingWithCustomers: bottlesOutstanding = 0,
  } = d.bottleStats ?? {};
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        {/* Live balance (not period-scoped) — how many empties still need to
            be collected from customers right now. */}
        <StatCard label="Pending Empty Recovery" value={String(bottlesOutstanding)} icon={PackageX} color="bg-rose-500/10" />
      </div>
