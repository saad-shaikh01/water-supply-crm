...
interface FleetPhotoUploadProps {
  onPhotosChange: (keys: string[]) => void;
  maxPhotos?: number;
  label?: string;
}

/** Generic Fleet photo capture/upload — same shape as driver/damage-photo-upload.tsx, reused for odometer/damage/receipt/invoice/document photos across Fleet. */
export function FleetPhotoUpload({ onPhotosChange, maxPhotos = 1, label = 'Photo' }: FleetPhotoUploadProps) {
  const [photos, setPhotos] = useState<UploadedPhoto[]>([]);
  const cameraRef = useRef<HTMLInputElement>(null);
  const galleryRef = useRef<HTMLInputElement>(null);

  const uploadedKeys = photos.filter((p) => !p.loading && p.key).map((p) => p.key);
  ...
  async function handleFiles(files: FileList | null) {
    ...
    const results = await Promise.allSettled(toProcess.map((f) => fleetApi.uploadPhoto(f)));
    ...
    <div className="space-y-2">
        ...
        <div className="grid grid-cols-3 gap-2">
          {photos.map((photo, i) => (
            <div key={i} className="relative aspect-square rounded-xl overflow-hidden border border-border bg-muted">
              ...
              <img src={photo.previewUrl} alt={`${label} ${i + 1}`} className="h-full w-full object-cover" />
              {photo.loading && (
      ...
      {canAddMore && (
        <div className="flex gap-2">
          <Button type="button" variant="outline" size="sm" className="flex-1 gap-2 rounded-xl h-11" onClick={() => cameraRef.current?.click()}>
            <Camera className="h-4 w-4" />
            Camera
          </Button>
          <Button type="button" variant="outline" size="sm" className="flex-1 gap-2 rounded-xl h-11" onClick={() => galleryRef.current?.click()}>
