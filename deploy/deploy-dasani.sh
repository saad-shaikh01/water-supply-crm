...
#   - docker-compose.dasani.yml (dasani-postgres:5433, dasani-redis:6380)
...
_validate_env() {
  ...
  # Dasani-specific: DATABASE_URL must point at port 5433 (dasani-postgres),
  ...
  if [ -n "${DATABASE_URL:-}" ] && echo "$DATABASE_URL" | grep -q ':5432/'; then
    ...
    echo "         Dasani must use port 5433 (dasani-postgres). Fix ${ENV_FILE}."
    errors=$((errors + 1))
  fi
...
_DB_URL="postgresql://${POSTGRES_USER}:${POSTGRES_PASSWORD}@127.0.0.1:5433/${POSTGRES_DB}?schema=public"
